...
dict_ac = {'zero': 0, 'one': 1, 'two': 2, 'three': 3, 'four': 4, 'five': 5}
dict_sign = set('plus', 'minus')

def calc(x, y, z):
    result = 0
    validation = (all(w in dict_ac for w in [x, z]) and y in dict_sign)
    if validation:
        if y == 'plus':
            result = dict_ac[x] + dict_ac[z]
        else:
            result = dict_ac[x] - dict_ac[z]
        sign = 'negative ' if result < 0 else ''
        print(f'{x} {y} {z} equals {sign}{dict_result[result]}')
    else:
        print("I am not able to answer this question. Check your input.")
